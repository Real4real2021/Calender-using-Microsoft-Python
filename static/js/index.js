document.addEventListener("DOMContentLoaded", function () {
  ...
  navLinks.forEach((link) => {
    ...
    function updateMarketAnalysis(data) {
      ...
      document.getElementById(
        "market-size-display"
      ).textContent = `Market Size: ${marketData.market_size || "N/A"}`;
      document.getElementById(
        "market-growth-rate-display"
      ).textContent = `Growth Rate: ${marketData.market_growth_rate || "N/A"}`;

      if (data.external_market_data) {
        const externalData = data.external_market_data;
        document.getElementById(
          "industry-growth-rate"
        ).textContent = `Industry Growth: ${
          externalData.industry_growth_rate || "N/A"
        }`;
      }
      ...
      if (marketData.market_trends && marketData.market_trends.length > 0) {
        for (let i = 0; i < marketData.market_trends.length; i++) {
          ...
          (listItem.textContent = trend.trend),
            trend.impact_on_product,
            trend.opportunity_or_threat;
    ...
    function updateMarketSizeChart(marketData) {
      ...
      const labels = marketData.market_segments
        ? Object.keys(marketData.market_segments)
        : [];
      const dataPoints = marketData.market_segments
        ? Object.values(marketData.market_segments)
        : [];
    ...
    function updateMarketGrowthChart(marketData) {
      const ctx = document
        .getElementById("market-growth-chart")
        .getContext("2d");
      if (charts.marketGrowthChart) charts.marketGrowthChart.destroy();

      const labels = marketData.historical_growth
        ? Object.keys(marketData.historical_growth)
        : [];
      const dataPoints = marketData.historical_growth
        ? Object.values(marketData.historical_growth)
        : [];
    ...
    function updateCompetitorAnalysis(data) {
      ...
      if (
        competitorData &&
        competitorData.customer_segmentation &&
        competitorData.customer_segmentation.length > 0
      ) {
        const row = document.createElement("tr");
        row.innerHTML = `
            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">${
              competitorData.competitor_name || "N/A"
            }</td>
            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">${
              competitorData.market_share || "N/A"
            }</td>
            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">${
              competitorData.customer_segmentation[0].segment_name || "N/A"
            }</td>
            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">${
              competitorData.customer_segmentation[0].segment_needs[0] || "N/A"
            }, ${
          competitorData.customer_segmentation[0].segment_needs[1] || "N/A"
        }</td>
            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">${
              competitorData.customer_segmentation[0]
                .segment_growth_potential || "N/A"
            }</td>
            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">${
              competitorData.customer_segmentation[0].segment_profitability ||
              "N/A"
            }</td>
            `;
      ...
      if (
        data.external_competitor_data &&
        Array.isArray(data.external_competitor_data)
      ) {
    ...
    function updateScenarioPlanning(data) {
      ...
      const projectedRevenueDisplay =
        document.getElementById("projected-revenue");
      const scenarioChartCanvas = document
        .getElementById("scenario-chart")
        .getContext("2d");
      let scenarioChart;
    ...
    function updateRecommendations(data) {
      ...
      if (
        data.cost_profit_analysis &&
        data.cost_profit_analysis.recommendations
      ) {
      ...
      if (
        data.revenue_break_even_analysis &&
        data.revenue_break_even_analysis.recommendations
      ) {
        allRecommendations.push({
      ...
      if (
        data.external_market_insights &&
        data.external_market_insights.recommendations
      ) {
        allRecommendations.push({
      ...
      allRecommendations.forEach((areaRecommendation) => {
        ...
        areaRecommendation.recommendations[0].forEach((recommendation) => {
          ...
          recDiv.innerHTML = `
            ...
            <p class="text-sm text-blue-600 mt-2">Impact: ${
            ...
            ${
              recommendation.priority
                ? `<p class="text-xs text-gray-500">Priority: ${
                    recommendation.priority
                  } | Feasibility: ${recommendation.feasibility || "N/A"}</p>`
                : ""
            }
        `;
    ...
    function updateCostChart(data) {
      ...
      if (data.cost_profit_analysis && data.cost_profit_analysis.costs) {
        if (data.cost_profit_analysis.costs.fixed_costs) {
          fixedCosts =
            data.cost_profit_analysis.costs.fixed_costs.total_fixed_costs || 0;
        }
        if (data.cost_profit_analysis.costs.variable_costs) {
          variableCosts =
            data.cost_profit_analysis.costs.variable_costs
    ...
    function updateRevenueChart(data) {
      ...
      if (data.cost_profit_analysis) {
        totalCosts =
          (data.cost_profit_analysis.total_cost_per_unit || 0) *
          (data.revenue_break_even_analysis.total_units_sold || 0);
      }
    ...
    function updateDashboard(data) {
      ...
      function updateProductInformation(data) {
        document.getElementById("product-id").textContent =
          data.product_information.product_id || "-";
        document.getElementById("product-name").textContent =
          data.product_information.product_name || "-";
        document.getElementById("product-category").textContent =
          data.product_information.product_category || "-";
        document.getElementById("launch-date").textContent =
          data.product_information.launch_date || "-";
        document.getElementById("target-market").textContent =
          data.product_information.target_market || "-";
        document.getElementById("lifecycle-stage").textContent =
          data.product_information.product_lifecycle_stage || "-";
      ...
      function updateRevenueBreakEvenAnalysis(revenueBreakEvenAnalysis) {
        ...
        document.getElementById("break-even-point-revenue").textContent =
          revenueBreakEvenAnalysis.break_even_point_revenue || "-";
        document.getElementById("break-even-point-units").textContent =
          revenueBreakEvenAnalysis.break_even_point_units || "-";
        document.getElementById("profit-margin").textContent =
          revenueBreakEvenAnalysis.profit_margin || "-";
        document.getElementById("total-profit").textContent =
          revenueBreakEvenAnalysis.total_profit || "-";
        document.getElementById("total-revenue").textContent =
          revenueBreakEvenAnalysis.total_revenue || "-";
        document.getElementById("total-units-sold").textContent =
          revenueBreakEvenAnalysis.total_units_sold || "-";
      }
      function updateMarketingPerformance(marketingPerformance) {
        if (!marketingPerformance) return;

        document.getElementById("marketing-roi").textContent =
          marketingPerformance.marketing_roi || "-";
      }
      ...
      function updateCustomerAnalysis(customerAnalysis) {
        ...
        document.getElementById("churn-rate").textContent =
          customerAnalysis.churn_rate || "-";
        document.getElementById("customer-acquisition-cost").textContent =
          customerAnalysis.customer_acquisition_cost || "-";
        document.getElementById("customer-lifetime-value").textContent =
          customerAnalysis.customer_lifetime_value || "-";
        document.getElementById("customer-retention-rate").textContent =
          customerAnalysis.customer_retention_rate || "-";
      }
      function updateCostProfitAnalysis(costProfitAnalysis) {
        ...
        document.getElementById("contribution-margin").textContent =
          costProfitAnalysis.contribution_margin || "-";
        document.getElementById("contribution-margin-ratio").textContent =
          costProfitAnalysis.contributino_margin_ratio || "-";
        document.getElementById("profit-per-unit").textContent =
          costProfitAnalysis.profit_per_unit || "-";
        document.getElementById("total-cost-per-unit").textContent =
          costProfitAnalysis.total_cost_per_unit || "-";
        document.getElementById("varibale-cost-per-unit").textContent =
          costProfitAnalysis.varibale_cost_per_unit || "-";
      }
      function updateSalesPerformance(salesPerformance) {
        ...
        document.getElementById("average-order-value").textContent =
          salesPerformance.average_order_value || "-";
        document.getElementById("sales-conversion-rate").textContent =
          salesPerformance.sales_conversion_rate || "-";
        document.getElementById("sales-cycle-length").textContent =
          salesPerformance.sales_cycle_length || "-";
        document.getElementById("sales-growth-rate").textContent =
          salesPerformance.sales_growth_rate || "-";
        document.getElementById("total-sales-revenue").textContent =
          salesPerformance.total_sales_revenue || "-";
      }

      function updateProductDevelopment(productDevelopment) {
        if (!productDevelopment) return;

        document.getElementById("development-cost").textContent =
          productDevelopment.development_cost || "-";
        document.getElementById("innovation-rate").textContent =
          productDevelopment.innovation_rate || "-";
        document.getElementById("new-feature-released").textContent =
          productDevelopment.new_feature_released || "-";
        document.getElementById("time-to-market").textContent =
          productDevelopment.time_to_market || "-";
      }

      function updateOperationalEfficiency(operationalEfficiency) {
        document.getElementById("defect-rate").textContent =
          data.operational_efficiency.defect_rate || "-";
        document.getElementById("inventory-turnover-rate").textContent =
          data.operational_efficiency.inventory_turnover_rate || "-";
        document.getElementById("order-fulfillment-rate").textContent =
          data.operational_efficiency.order_fulfillment_time || "-";
        document.getElementById("product-capacity-utilization").textContent =
          data.operational_efficiency.production_capacity_utilization || "-";
        document.getElementById("return-rate").textContent =
          data.operational_efficiency.return_rate || "-";
        document.getElementById("supply-chain-costs").textContent =
          data.operational_efficiency.supply_chain_costs || "-";
      }
      ...
      function updateRiskAssessment(riskAssessment) {
        ...
        // console.log(data)
        document.getElementById("average-order-value").textContent =
          data.sales_performance.average_order_value || "-";
        document.getElementById("sales-conversion-rate").textContent =
          data.sales_performance.sales_conversion_rate || "-";
        document.getElementById("sales-cycle-length").textContent =
          data.sales_performance.sales_cycle_length || "-";
        document.getElementById("sales-growth-rate").textContent =
          data.sales_performance.sales_growth_rate || "-";
        document.getElementById("churn-rate").textContent =
          data.customer_analysis.churn_rate || "-";
        document.getElementById("customer-acquisition-cost").textContent =
          data.customer_analysis.customer_acquisition_cost || "-";
        document.getElementById("overall-effectiveness").textContent =
          data.risk_assessment.risk_mitigation_effectiveness
    ...
    fileInput.addEventListener("change", function () {
      if (this.files.length > 0) {
        ...
        if (
          prevFileName &&
          prevFileName !== fileUpload.querySelector("p:first-of-type")
        ) {
          prevFileName.remove();
        }
  ...
  document
    .getElementById("industry-button")
